...
const Home = () => {
    ...
    useEffect(() => {
        async function getTripData() {
            try {
                const tripResponse = await axios.get("http://localhost:5000/")
    ...
    async function searchQuery(e) {
        ...
        try {
            const response = await axios.post("http://localhost:5000/search", {location: search});
        ...
        <div className='content'>
            ...
            <div className='search'>
                <form onSubmit={(e) => searchQuery(e)}>
                    ...
                    <input className='searchField' type="search" name="search" value={search} onChange={(e) => setSearch(e.target.value)}></input>
                    ...
                    {tripData.map(trip => (
                        trip.username !== username && (
                            <div className='gridItems' key={trip.tripID}>
                                ...
                                <button className='interestBtn' type="submit" onClick={(e) => addInterest(e, trip.tripID)}>Interested</button>
                            </div>
                            )                
                        ))}
                </div>
